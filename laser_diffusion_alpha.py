...
def filter_rgb_weight(Im,a=20,b=1,c=1):
    ...
    return I_av
def Image_norm(I_av,pixel_num=10,area=50):
    
    div = int(np.shape(I_av)[1]/area)
    IBinary = I_av.copy()

    for i in range(0,area+1,1):
        Ilinear = -np.sort(np.unique(-I_av[:,(i*div):((i+1)*div)].ravel()))
        if Ilinear[pixel_num]>0.8:
            IBinary[:,(i*div):((i+1)*div)] = (I_av[:,(i*div):((i+1)*div)] > Ilinear[pixel_num])
        elif Ilinear[pixel_num]<0.8:
            IBinary[:,(i*div):((i+1)*div)] = 0
    IBinary[:,((i+1)*div):-1] = 0
    IBinary[:,-1] = 0
...
def fix_skewed(x,y):
    ...
    maxxloc = np.where(np.max(y)==y)[0]
    lhsloc = np.where(x<=x[maxxloc])
    rhsloc = np.where(x>=x[maxxloc])
    def slopefinder(mm):
        bb = 0
        xsolve = x[loc]+((y[loc]-bb)/mm)
        return xsolve
    
    def slopeobjective(mm):
        xsolve = slopefinder(mm)
        xmax = 0.5
        localmax = np.where(np.max(y)==y)[0]
    
        return abs(xsolve[localmax]-xmax)[0]
         
    xplot = x.copy()
    yplot = y.copy()
    loc = lhsloc
    sol1 = fsolve(slopeobjective,[0.1])
    xplot[lhsloc] = slopefinder(sol1)
    diff = 0.5-x[maxxloc]
    x[rhsloc] = diff+x[rhsloc]
    loc = rhsloc
    sol2 = fsolve(slopeobjective,[0.1])
    xplot[rhsloc] = slopefinder(sol2)
    return xplot,yplot
...
def diffuse_fit(x,y,ti,t0):
    dt = ti-t0
    popt,pcov = curve_fit(pde_fun,x,y,p0 = [1 ,-1,-1])
    C1,C2,C3 = popt
